...
const AnimatedTicker = memo(({ 
  ...
  return (
    <div className="absolute top-0 left-0 right-0 overflow-hidden bg-black/20 sm:bg-black/30 backdrop-blur-sm border-b border-white/5 z-20" style={{ contain: 'layout' }}>
      ...
      <div className="auth-ticker flex whitespace-nowrap py-1.5 sm:py-2 pl-16 sm:pl-20">
        {[...tickers, ...tickers, ...tickers].map((ticker, i) => (
          <div 
            key={`${ticker.symbol}-${i}-${flashKey}`} 
            className="flex items-center gap-2 sm:gap-4 md:gap-6 mx-4 sm:mx-6 md:mx-8"
          >
            <span className="text-gray-400 text-xs sm:text-sm font-medium">{ticker.symbol}</span>
            <span className={cn(
              "text-white text-xs sm:text-sm font-bold tabular-nums transition-all duration-300",
              !loading && "auth-price-flash"
            )}>
              ${ticker.price}
            </span>
            <span className={cn(
      ...
      {lastUpdate && (
        <div className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-gray-600 hidden md:block">
...
export default function Auth() {
  ...
  return (
    <div className="auth-container auth-safe-area flex flex-col lg:flex-row relative overflow-hidden bg-[#0a0b0f] text-white selection:bg-emerald-500/30">
      ...
      {/* Live Ticker - Now with real-time data */}
      <AnimatedTicker 
        tickers={tickers} 
        loading={marketLoading} 
