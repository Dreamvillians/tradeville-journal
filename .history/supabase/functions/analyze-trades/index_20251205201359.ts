...
serve(async (req: { method: string; headers: { get: (arg0: string) => any; }; json: () => PromiseLike<{ period: any; }> | { period: any; }; }) => {
  ...
  try {
    ...
    const profitableTrades = trades.filter((t: { profit_loss_currency: any; }) => (t.profit_loss_currency || 0) > 0).length;
    const winRate = ((profitableTrades / totalTrades) * 100).toFixed(1);
    const totalProfit = trades.filter((t: { profit_loss_currency: any; }) => (t.profit_loss_currency || 0) > 0)
      .reduce((sum: any, t: { profit_loss_currency: any; }) => sum + (t.profit_loss_currency || 0), 0);
    const totalLoss = Math.abs(trades.filter((t: { profit_loss_currency: any; }) => (t.profit_loss_currency || 0) < 0)
      .reduce((sum: any, t: { profit_loss_currency: any; }) => sum + (t.profit_loss_currency || 0), 0));
    const profitFactor = totalLoss > 0 ? (totalProfit / totalLoss).toFixed(2) : "∞";
    const netPnL = trades.reduce((sum: any, t: { profit_loss_currency: any; }) => sum + (t.profit_loss_currency || 0), 0);

    // Group by strategy
    const strategyPerformance = trades.reduce((acc: any, trade: { strategies: { name: string; }; profit_loss_currency: any; }) => {
    ...
    // Group by instrument
    const instrumentPerformance = trades.reduce((acc: any, trade: { instrument: string; profit_loss_currency: any; }) => {
    ...
    const recentTrades = trades.slice(0, 10).map((t: { instrument: any; direction: any; profit_loss_currency: any; strategies: { name: any; }; opened_at: any; }) => ({
...
${recentTrades.map((t: { date: string; direction: any; instrument: any; strategy: any; pnl: number; }) => 
