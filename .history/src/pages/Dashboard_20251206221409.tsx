...
import { useState, useEffect, useMemo, useCallback, memo } from "react"; // 1. Added 'memo' to imports
...
interface DashboardStats {
  ...
  avgWin: number;
  avgLoss: number;
  avgRiskReward: number; // New
  avgTradeDuration: string; // New
  expectancy: number;
  bestTrade: number;
  worstTrade: number;
  streak: number;
...
// New helper to format millisecond duration into readable string
                ...
                <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-emerald-500/20 to-blue-500/20 flex items-center justify-center">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-[#12131a] flex items-center justify-center">
                    <div className="text-center">
                      <p className={cn("text-xl sm:text-2xl font-bold", parseFloat(stats.winRate) >= 50 ? "text-emerald-400" : "text-red-400")}>
                        {stats.winRate}%
                      </p>
            ...
            <div className="relative">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-500/20 to-blue-500/20 flex items-center justify-center">
                <div className="w-16 h-16 rounded-full bg-[#12131a] flex items-center justify-center">
                  <div className="text-center">
                    <p className={cn("text-xl font-bold", parseFloat(stats.winRate) >= 50 ? "text-emerald-400" : "text-red-400")}>
                      {stats.winRate}%
                    </p>
      ...
      <CardContent className="relative z-10 pt-2 sm:pt-4 px-3 sm:px-6 pb-3 sm:pb-6">
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
          <div className="p-2 sm:p-3 rounded-lg bg-white/5">
            ...
            <p className={cn("text-sm sm:text-lg font-bold", periodReturn >= 0 ? "text-emerald-400" : "text-red-400")}>{formatCurrency(periodReturn)}</p>
          </div>
          <div className="p-2 sm:p-3 rounded-lg bg-white/5">
            <p className="text-[10px] sm:text-xs text-gray-500 mb-0.5 sm:mb-1">Best Day</p>
            <p className="text-sm sm:text-lg font-bold text-emerald-400">{formatCurrency(bestDay)}</p>
          </div>
          <div className="p-2 sm:p-3 rounded-lg bg-white/5">
            <p className="text-[10px] sm:text-xs text-gray-500 mb-0.5 sm:mb-1">Worst Day</p>
            <p className="text-sm sm:text-lg font-bold text-red-400">{formatCurrency(worstDay)}</p>
          </div>
        </div>
            ...
            <>
              ...
              <div className="ml-12 sm:ml-16 h-full relative">
                <div className="absolute inset-0">
                  {[0, 1, 2, 3, 4].map((i) => (<div key={i} className="absolute w-full border-t border-white/5" style={{ top: `${i * 25}%` }} />))}
                </div>
                    ...
                    <div key={i} className="flex-1 h-full relative cursor-crosshair" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
                      {hoveredIndex === i && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 bg-[#1a1b23] border border-white/10 rounded-lg shadow-xl z-20 whitespace-nowrap">
                          ...
                          <p className="text-xs sm:text-sm font-bold text-white">{formatCurrency(d.value)} (Equity)</p>
    ...
    <Card className="dash-glass dash-card-hover relative overflow-hidden h-full">
      ...
      <CardHeader className="relative z-10 pb-2 px-3 sm:px-6 pt-3 sm:pt-6">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button variant="ghost" size="sm" className="text-gray-400 hover:text-white flex-shrink-0 text-xs sm:text-sm px-2 sm:px-3" onClick={() => navigate("/journal")}>
        ...
        <div className="space-y-2 sm:space-y-3">
          {recentTrades.length > 0 ? (
            recentTrades.map((trade, index) => (
              <div key={trade.id} className="flex items-center justify-between p-2 sm:p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-all group cursor-pointer dash-slide-right gap-2" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-white group-hover:text-emerald-400 transition-colors text-sm sm:text-base truncate">{trade.symbol}</p>
                    <p className="text-[10px] sm:text-xs text-gray-500 truncate">{trade.strategy || "Manual Trade"}</p>
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-500">{getTimeAgo(trade.closed_at)}</p>
                </div>
              </div>
            ))
          ) : (
            <div className="text-center py-6 sm:py-8">
              ...
              <p className="text-xs sm:text-sm text-gray-500">Start logging your trades to see them here</p>
            </div>
          )}
        </div>
      </CardContent>
      ...
      <CardContent className="relative z-10 pt-1 sm:pt-2 pb-3 sm:pb-4 px-3 sm:px-4">
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {actions.map((action, index) => (
            <Button key={action.label} variant={action.primary ? "default" : "outline"} className={cn("h-auto py-2.5 sm:py-3 md:py-4 flex flex-col items-center gap-1 sm:gap-1.5 md:gap-2 group transition-all w-full", action.primary ? "bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 shadow-lg shadow-emerald-500/25" : "bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20")} onClick={() => onNavigate(action.path)} style={{ animationDelay: `${index * 50}ms` }}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#12131a] border-white/10 text-white max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">Set your monthly trading targets to track your progress.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="space-y-2"><Label htmlFor="pnl-goal" className="text-gray-300">Monthly P&L Goal ($)</Label><Input id="pnl-goal" type="number" value={localGoals.monthly_pnl_goal} onChange={(e) => setLocalGoals(prev => ({ ...prev, monthly_pnl_goal: parseFloat(e.target.value) || 0 }))} className="bg-white/5 border-white/10 text-white focus:border-emerald-500/50" placeholder="5000" /><p className="text-xs text-gray-500">Your target profit for the month</p></div>
          <div className="space-y-2"><Label htmlFor="trades-goal" className="text-gray-300">Monthly Trades Goal</Label><Input id="trades-goal" type="number" value={localGoals.monthly_trades_goal} onChange={(e) => setLocalGoals(prev => ({ ...prev, monthly_trades_goal: parseInt(e.target.value) || 0 }))} className="bg-white/5 border-white/10 text-white focus:border-emerald-500/50" placeholder="20" /><p className="text-xs text-gray-500">Number of trades you aim to execute</p></div>
          <div className="space-y-2"><Label htmlFor="winrate-goal" className="text-gray-300">Win Rate Goal (%)</Label><Input id="winrate-goal" type="number" min="0" max="100" value={localGoals.win_rate_goal} onChange={(e) => setLocalGoals(prev => ({ ...prev, win_rate_goal: Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)) }))} className="bg-white/5 border-white/10 text-white focus:border-emerald-500/50" placeholder="60" /><p className="text-xs text-gray-500">Target win percentage (0-100%)</p></div>
        </div>
        <DialogFooter className="gap-2"><Button variant="outline" onClick={() => onOpenChange(false)} className="bg-white/5 border-white/10 text-gray-300 hover:bg-white/10"><X className="w-4 h-4 mr-2" />Cancel</Button><Button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-700 text-white"><Save className="w-4 h-4 mr-2" />Save Goals</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
});
...
const TradingGoals = memo(({ stats, trades, goals, onEditGoals }: { stats: DashboardStats; trades: Trade[]; goals: UserGoals; onEditGoals: () => void; }) => {
  ...
  return (
    <Card className="dash-glass dash-card-hover relative overflow-hidden h-full">
      ...
      <CardHeader className="relative z-10 pb-2 px-3 sm:px-6 pt-3 sm:pt-6">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-base sm:text-lg text-white flex items-center gap-2"><Target className="w-4 h-4 sm:w-5 sm:h-5 text-orange-400" /><span className="truncate">Trading Goals</span></CardTitle>
          <Button variant="ghost" size="sm" className="text-gray-400 hover:text-white p-1.5 sm:p-2" onClick={onEditGoals}><Settings className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></Button>
        </div>
      </CardHeader>
          ...
          return (
            <div key={goal.label} className="space-y-1.5 sm:space-y-2">
              <div className="flex items-center justify-between text-xs sm:text-sm">
                <span className="text-gray-400 truncate flex items-center gap-1">{goal.label}{isComplete && <Award className="w-3 h-3 text-yellow-400" />}</span>
                <span className={cn("font-medium text-[11px] sm:text-sm flex-shrink-0 ml-2", goal.current >= 0 ? "text-white" : "text-red-400")}>{goal.unit ? `${goal.current.toFixed(1)}${goal.unit}` : formatCurrency(goal.current)} / {goal.unit ? `${goal.target}${goal.unit}` : formatCurrency(goal.target)}</span>
              </div>
              <div className="h-1.5 sm:h-2 bg-white/10 rounded-full overflow-hidden">
              ...
              <p className="text-[10px] text-gray-500 text-right">{progress.toFixed(0)}% {isComplete ? "✓ Complete!" : "complete"}</p>
            </div>
          );
        })}
      </CardContent>
...
const PerformanceMetrics = memo(({ stats }: { stats: DashboardStats }) => {
  ...
  return (
    <Card className="dash-glass dash-card-hover relative overflow-hidden h-full">
      ...
      <CardHeader className="relative z-10 pb-2 px-3 sm:px-6 pt-3 sm:pt-6">
        <CardTitle className="text-base sm:text-lg text-white flex items-center gap-2"><PieChart className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400" /><span className="truncate">Performance Metrics</span></CardTitle>
      </CardHeader>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
          {metrics.map((metric) => (
            <div key={metric.label} className="p-2 sm:p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-all group">
              ...
              <p className={cn("text-sm sm:text-lg font-bold truncate", metric.trend === "up" ? "text-emerald-400" : metric.trend === "down" ? "text-red-400" : "text-white")}>{metric.value}</p>
            </div>
          ))}
        </div>
      </CardContent>
      ...
      <div className={cn("relative z-10 pt-4 sm:pt-6 pb-6 sm:pb-8 px-3 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full transition-all duration-700", isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
        <div className="flex items-center justify-between mb-6 sm:mb-8 gap-3">
          <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
            ...
            <div className="min-w-0"><h1 className="text-xl sm:text-2xl font-bold text-white truncate">Dashboard</h1><p className="text-xs sm:text-sm text-gray-500 truncate">Real-time trading analytics</p></div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
            <Button variant="outline" size="sm" className="bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hidden sm:flex" onClick={handleRefresh}><RefreshCw className="w-4 h-4 sm:mr-2" /><span className="hidden md:inline">Sync</span></Button>
            <Button variant="outline" size="sm" className="bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 p-2 sm:p-2.5"><Bell className="w-4 h-4" /></Button>
            <Button variant="outline" size="sm" className="bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 p-2 sm:p-2.5"><Settings className="w-4 h-4" /></Button>
          </div>
        </div>
