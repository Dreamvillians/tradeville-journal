...
const WelcomeCard = memo(({ userName, stats }: { userName: string; stats: DashboardStats }) => {
  ...
  const isMarketOpen = useMemo(() => {
    ...
    // Simplified: Mon-Fri, 9am - 5pm UTC (approx)
        ...
        <div className="flex flex-col gap-4 sm:gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="hidden sm:flex items-center gap-4 flex-shrink-0">
              <div className="relative">
                ...
                <div className="absolute -top-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-emerald-500 flex items-center justify-center dash-bounce">
          ...
          <div className="sm:hidden flex items-center justify-center">
            <div className="relative">
              ...
              <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center dash-bounce">
...
const PerformanceChart = memo(({ trades }: { trades: Trade[] }) => {
  ...
  const chartData = useMemo(() => {
    if (trades.length === 0) {
      // Return empty placeholder data if no trades
    ...
    // 2. Filter trades by date and sort Ascending for calculation
    ...
    // Fill gaps if needed, but for now just map actual trading days
    filteredTrades.forEach(t => {
      const dateObj = new Date(t.closed_at || t.opened_at);
      const dateStr = dateObj.toLocaleDateString("en-US", { month: "short", day: "numeric" });
      
      if (!dailyMap.has(dateStr)) {
        dailyMap.set(dateStr, { pnl: 0, date: dateStr });
      }
    ...
    const data = Array.from(dailyMap.values()).map(day => {
      ...
      return {
        date: day.date,
        value: cumulative, // This is total P&L over the period
        pnl: day.pnl,
        isProfit: day.pnl >= 0
      };
    });

    // Ensure we have at least a few points for the chart to render
    if (data.length < 2) {
        data.unshift({ date: "Start", value: 0, pnl: 0, isProfit: true });
    }
  ...
  // Calculate Return for the specific period
  ...
  const goals = [
    {
      ...
      target: 5000, // TODO: Fetch from user settings
      color: "emerald",
    },
    { label: "Win Rate", current: parseFloat(stats.winRate), target: 60, unit: "%", color: "blue" },
    {
      label: "Trades/Month",
      current: monthlyStats.count,
      target: 20, // TODO: Fetch from user settings
      color: "purple",
    },
  ];
          ...
          return (
            <div key={goal.label} className="space-y-1.5 sm:space-y-2">
              ...
              <div className="h-1.5 sm:h-2 bg-white/10 rounded-full overflow-hidden">
                <div className={cn("h-full rounded-full transition-all duration-1000 ease-out", colorClasses[goal.color as keyof typeof colorClasses])} style={{ width: `${Math.min(progress, 100)}%` }} />
              </div>
            </div>
          );
        })}
...
const Dashboard = () => {
  ...
  const stats = useMemo((): DashboardStats => {
    ...
    if (trades.length > 0) {
      ...
      // If it's a loss streak, we can represent it as negative, or just absolute number. 
      // For "Win Streak" display, we might want to only show positive, or handle logic in UI.
      // Let's assume visual streak implies wins for "Flame" icon, or reset to 0 if loss.
      ...
      <div className={cn("relative z-10 pt-4 sm:pt-6 pb-6 sm:pb-8 px-3 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full transition-all duration-700", isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4")}>
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8 gap-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
            <Button variant="outline" size="sm" className="bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hidden sm:flex">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          ...
          <div className="col-span-1 sm:col-span-2 dash-slide-up" style={{ animationDelay: "100ms" }}>
            <StatCard
              ...
              trendValue={formatPercent((stats.totalPnL / (10000 || 1)) * 100)} // Assuming 10k starting balance for % calc context
              subtitle="All time"
          ...
          <div className="dash-slide-up" style={{ animationDelay: "300ms" }}>
            <StatCard title="Current Streak" value={stats.streak} icon={Flame} trend="up" subtitle="Consecutive wins" color="orange" />
          </div>
