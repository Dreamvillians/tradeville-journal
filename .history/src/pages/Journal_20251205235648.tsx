...
const JOURNAL_STYLES = `
  ...
  .journal-row:hover .sticky-col {
    background: #13141c; /* Slightly lighter on hover */
  }
  ...
  .custom-scrollbar::-webkit-scrollbar-thumb {
    ...
    border-radius: 10px;
    border: 3px solid rgba(15, 16, 24, 1); /* Matches bg to create padding effect */
  }
  .custom-scrollbar::-webkit-scrollbar-thumb:hover {
    background: rgba(52, 211, 153, 0.4); /* Emerald hover */
  }
  ...
  .sticky-col {
    ...
    left: 0;
    z-index: 20;
    background: #0a0b0f; /* Must match main bg to hide scrolling content behind it */
    transition: background-color 0.2s;
    box-shadow: 2px 0 10px rgba(0,0,0,0.2); /* Shadow to separate sticky col */
  }
`;
...
// Mobile Trade Card
const MobileTradeCard = memo(({ trade, onViewDetails }: { trade: Trade, onViewDetails: (t: Trade) => void }) => {
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={cn("w-1 h-8 rounded-full", trade.direction === "LONG" ? "bg-emerald-500" : "bg-rose-500")} />
          <div>
            <div className="font-bold text-white">{trade.instrument}</div>
            <div className="text-xs text-gray-500">{format(new Date(trade.opened_at), "MMM dd")}</div>
          </div>
        </div>
        <div className={cn(
          "font-mono font-bold",
          isWin ? "text-emerald-400" : (trade.profit_loss_currency || 0) < 0 ? "text-rose-400" : "text-gray-400"
        )}>
          {trade.profit_loss_currency ? `$${trade.profit_loss_currency}` : "OPEN"}
        </div>
      </div>
    </div>
  );
...
const Journal = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0b0f] text-white relative overflow-hidden font-sans selection:bg-emerald-500/30">
      <FloatingOrbs />

      {/* Container with max-width to prevent sidebar overlap */}
      <div className="relative z-10 w-full max-w-[100vw] mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 animate-enter">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-6 sm:mb-8">
          <StatBadge label="Net P&L" value={`$${stats.netPnL.toFixed(2)}`} icon={Wallet} color={stats.netPnL >= 0 ? "emerald" : "rose"} />
          <StatBadge label="Win Rate" value={`${stats.winRate.toFixed(1)}%`} icon={Activity} color="blue" />
          <StatBadge label="Open" value={stats.openPositions} icon={Clock} color="default" />
          <StatBadge label="Total" value={stats.total} icon={TrendingUp} color="default" />
        </div>
            ...
            {(["ALL", "OPEN", "CLOSED"] as FilterStatus[]).map((status) => (
              <button
                ...
                className={cn(
                  ...
                  statusFilter === status ? "bg-white/10 text-white shadow-sm" : "text-gray-500 hover:text-gray-300"
                )}
              >
                {status}
              </button>
        ...
        {/* Main Table */}
        <Card className="journal-glass border-none overflow-hidden w-full max-w-full">
          <CardContent className="p-0">
            {loading ? (
              <div className="flex justify-center py-20"><div className="animate-spin h-6 w-6 border-2 border-emerald-500 border-t-transparent rounded-full" /></div>
            ) : filteredTrades.length === 0 ? (
              <div className="text-center py-20 text-gray-500">No trades found</div>
            ) : (
              <>
                {/* Mobile List */}
                <div className="block lg:hidden p-3 space-y-3">
                  {filteredTrades.map((trade) => (
                    <MobileTradeCard key={trade.id} trade={trade} onViewDetails={setSelectedTrade} />
                  ))}
                </div>
                  ...
                  <Table className="w-full min-w-[1200px]">
                    <TableHeader className="bg-white/[0.02] border-b border-white/5 sticky top-0 z-30 backdrop-blur-sm">
                      <TableRow className="hover:bg-transparent border-none">
                        {/* STICKY HEADER COLUMN 1: Instrument */}
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider sticky-col pl-6">Instrument</TableHead>
                        
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center w-[80px]">Status</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center w-[80px]">Result</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center w-[80px]">Direction</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Opened</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right">Entry</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Closed</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right">Exit</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right">Size</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right">P&L</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Setup</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center">Screenshots</TableHead>
                        <TableHead className="h-10 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-center pr-4">View</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredTrades.map((trade) => {
                        ...
                        return (
                          <TableRow key={trade.id} className="journal-row border-b border-white/5 transition-colors text-xs group">
                            
                            {/* 1. STICKY COLUMN: Instrument */}
                            <TableCell className="sticky-col font-bold text-white whitespace-nowrap pl-6">
                              {trade.instrument}
                            </TableCell>
                            ...
                            <TableCell className="text-center">
                              <Badge variant="outline" className={cn(
                                ...
                                trade.closed_at ? "text-gray-400 border-gray-500" : "text-blue-400 border-blue-500 bg-blue-500/10"
                              )}>
                            ...
                            <TableCell className="text-center">
                              {trade.closed_at && (
                                <Badge variant="outline" className={cn(
                                  "text-[9px] px-1.5 py-0 h-5 border-opacity-30",
                                  isWin ? "text-emerald-400 border-emerald-500 bg-emerald-500/10" : 
                                  isLoss ? "text-rose-400 border-rose-500 bg-rose-500/10" : 
                            ...
                            {/* 11. Setup */}
                            <TableCell className="text-gray-400 truncate max-w-[100px]" title={trade.setup_type || ""}>
                              {trade.setup_type || "-"}
                            </TableCell>
                            ...
                            <TableCell className="text-center">
                              <div className="flex justify-center -space-x-2 hover:space-x-1 transition-all">
                                {hasScreenshots ? trade.trade_images.slice(0, 2).map((img, i) => (
                                  <div 
                                    key={img.id}
      ...
      <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
        <DialogContent className="w-[95vw] max-w-6xl bg-black/95 border-white/10 p-0 overflow-hidden">
          <div className="relative w-full h-[80vh] flex items-center justify-center">
            {selectedImage && (
              <img src={selectedImage} alt="Trade" className="max-w-full max-h-full object-contain" />
            )}
            <button 
        ...
        <DialogContent className="w-[95vw] max-w-md bg-[#0F1018] border border-white/10 text-white">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-3">
              <span className="text-xl font-bold">{selectedTrade?.instrument}</span>
              <Badge className={cn(
                "text-xs",
                selectedTrade?.direction === "LONG" ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"
              )}>
            ...
            <div className="space-y-6 py-4">
              <div className="text-center p-4 bg-white/5 rounded-xl border border-white/5">
                ...
                <div className={cn(
                  ...
                  (selectedTrade.profit_loss_currency || 0) > 0 ? "text-emerald-400" : 
                  (selectedTrade.profit_loss_currency || 0) < 0 ? "text-rose-400" : "text-gray-400"
                )}>
                  {selectedTrade.profit_loss_currency ? `$${selectedTrade.profit_loss_currency.toFixed(2)}` : "OPEN"}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="space-y-1">
                  ...
                  <div className="font-mono">{format(new Date(selectedTrade.opened_at), "MMM dd, HH:mm")}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-gray-500 text-xs">Closed</div>
                  <div className="font-mono">{selectedTrade.closed_at ? format(new Date(selectedTrade.closed_at), "MMM dd, HH:mm") : "-"}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-gray-500 text-xs">Entry Price</div>
                  <div className="font-mono">{selectedTrade.entry_price}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-gray-500 text-xs">Exit Price</div>
                  <div className="font-mono">{selectedTrade.exit_price || "-"}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-gray-500 text-xs">Size</div>
                  <div className="font-mono">{selectedTrade.position_size}</div>
                </div>
                <div className="space-y-1">
