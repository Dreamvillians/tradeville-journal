...
const FEATURES = [
  {
    ...
    description: "Sync trades from 50+ brokers instantly.",
  },
  {
    icon: BarChart3,
    title: "Detailed Analytics",
    description: "Visualize R-multiples, win rates, and P&L.",
  },
  {
    icon: Zap,
    title: "Strategy Tracking",
    description: "Identify profitable and losing setups.",
  },
  {
    icon: Shield,
  ...
  const fetchMarketData = useCallback(async () => {
    ...
    try {
      ...
      const newTickers: TickerData[] = [
        ...
        {
          symbol: "SOL/USD",
          price: solPrice.toLocaleString("en-US", {
      ...
      if (
        candleResponse.status === "fulfilled" &&
        candleResponse.value.ok
      ) {
        try {
            ...
            const newCandles: CandleData[] = klines.map(
              (kline: any[], index: number) => {
                ...
                return {
                  ...
                  height: Math.max(
                    20,
                    Math.min(70, baseHeight + dynamicHeight)
                  ),
                  isGreen,
                  delay: index * 0.15,
                  wickTop: Math.max(
                    3,
                    Math.min(15, wickTopSize + 3)
                  ),
                  wickBottom: Math.max(
                    3,
                    Math.min(15, wickBottomSize + 3)
                  ),
                  open,
                  close,
                  high,
        ...
        const generatedCandles: CandleData[] = Array.from(
          ...
          (_, index) => {
            ...
            return {
              id: index,
              height: Math.min(
                Math.max(baseHeight + randomHeight, 20),
                65
              ),
              isGreen,
              delay: index * 0.15,
...
const AUTH_STYLES = `
  ...
  /* Root container: full width, no horizontal scroll */
  .auth-container { 
    min-height: 100vh; 
    min-height: 100dvh;
    width: 100%;
  ...
  /* Helper wrapper around scrollable content */
  .auth-scroll {
    width: 100%;
    max-width: 100%;
  }
  
  /* Hide heavy visuals on small screens */
  ...
  /* Custom scrollbar for any scrollable child */
      ...
      {PARTICLES.map((p) => (
        <div
          ...
          style={{
            ...
            top: `${p.top}%`,
            background: `rgba(${
              p.isGreen ? "16, 185, 129" : "59, 130, 246"
            }, ${p.opacity})`,
...
const AnimatedCandlesticks = memo(
  ({ candles, loading }: { candles: CandleData[]; loading: boolean }) => {
    return (
      <div
        className="absolute bottom-0 left-0 right-0 h-24 sm:h-32 md:h-40 lg:h-48 hidden sm:flex items-end justify-center gap-2 md:gap-3 opacity-15 sm:opacity-20 overflow-hidden"
        style={{ contain: "strict" }}
      >
        {loading && (
...
const AnimatedLogo = memo(
  ({ size = "default" }: { size?: "default" | "large" | "small" }) => {
    const sizeClasses = {
      small: "w-10 h-10",
  ...
  ({ value, label }: { value: string; label: string }) => {
    ...
    return (
      <div className="p-2.5 sm:p-3 md:p-4 rounded-lg sm:rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 text-center group hover:bg-white/10 hover:border-emerald-500/30 transition-all duration-300">
        <div className="text-lg sm:text-xl md:text-2xl font-bold text-emerald-400 tabular-nums">
          {displayValue}
        </div>
  ...
  ({
    ...
    >
      ...
      <div className="min-w-0">
        <h3 className="	font-semibold text-white text-sm md:text-base mb-0.5 group-hover:text-emerald-400 transition-colors">
          {title}
        </h3>
      ...
      <div className="relative w-full group">
        ...
        <Button
          ...
          className={cn(
            ...
            isLastUsed &&
              "border-emerald-500/50 ring-1 sm:ring-2 ring-emerald-500/20 bg-emerald-500/5"
          )}
        >
...
export default function Auth() {
  ...
  return (
    <div className="auth-container auth-safe-area flex flex-col lg:flex-row relative bg-[#0a0b0f] text-white selection:bg-emerald-500/30">
      {/* Background layers */}
      <FloatingOrbs />
      <AnimatedGrid />
      ...
      <div className="hidden lg:flex lg:w-1/2 xl:w-[55%] flex-col justify-center p-6 lg:p-8 xl:p-12 relative z-10 border-r border-white/5">
        ...
        >
          {/* Logo/Brand */}
          <div
            className="space-y-4 xl:space-y-6 auth-fade"
            style={{ animationDelay: "100ms" }}
          >
            <div className="flex items-center gap-3 xl:gap-4">
              <AnimatedLogo />
              <div>
                <span className="text-2xl xl:text-3xl font-bold tracking-tight bg-gradient-to-r from-white via-white to-gray-400 bg-clip-text text-transparent">
                  Tradeville
                </span>
                <div className="text-xs xl:text-sm text-emerald-400 font-medium tracking-wider uppercase">
                  Trading Journal
                </div>
              </div>
            </div>
            ...
            <p className="text-sm lg:text-base xl:text-lg text-gray-400 max-w-md leading-relaxed">
              Stop trading blindly. Track, analyze, and improve your
              trading performance with the most advanced journaling
              platform.
            </p>
          </div>
          ...
          {/* Animated Stats */}
          <div
            className="grid grid-cols-3 gap-2 lg:gap-3 xl:gap-4 auth-fade"
          ...
          >
            ...
            <p className="text-gray-300 italic text-sm xl:text-base relative z-10">
              "Tradeville helped me cut my losing days by 40% in the
              first month."
            </p>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 md:p-8 relative z-10 auth-scroll w-full">
        <Card
          className={cn(
            "w-full max-w-[420px] sm:max-w-md mx-auto border-white/10 bg-[#12131a]/95 sm:bg-[#12131a]/90 backdrop-blur-xl shadow-2xl transition-all duration-700 transform relative overflow-hidden",
            isVisible
        ...
        >
          ...
          <CardContent className="pt-4 sm:pt-6 px-4 sm:px-6 pb-4 relative z-10">
            ...
            >
              ...
              <TabsContent
                value="signin"
                className="space-y-4 mt-5 sm:mt-6 auth-fade"
              >
                <form
                ...
                >
                  <div className="space-y-2">
                    <Label
                      ...
                      className="text-gray-300 text-xs uppercase tracking-wider font-semibold"
                    >
                      Email
                    </Label>
                    <AnimatedInput
                ...
                <div className="relative py-3">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-[#12131a] px-3 text-gray-500">
                      Or continue with
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {/* Row 1: 3 main providers on all screens */}
                  <div className="grid grid-cols-3 gap-2 sm:gap-3">
                    {SOCIAL_BUTTONS.slice(0, 3).map((btn) => (
                      <SocialButton
                        ...
                        disabled={loading}
                        isLastUsed={
                          lastUsedProvider === btn.provider
                        }
                      />
                    ))}
                  </div>
                  {/* Row 2: extra providers */}
              ...
              >
                ...
                {/* Social Buttons - same layout as sign-in */}
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-2 sm:gap-3">
                    {SOCIAL_BUTTONS.slice(0, 3).map((btn) => (
                      <SocialButton
