...
// Initial static data - will be replaced with real data
const INITIAL_TICKERS = [
  { symbol: "BTC/USD", price: "---", change: "0.0%", isUp: true },
  { symbol: "ETH/USD", price: "---", change: "0.0%", isUp: true },
  { symbol: "EUR/USD", price: "---", change: "0.0%", isUp: false },
  { symbol: "AAPL", price: "---", change: "0.0%", isUp: true },
];

const INITIAL_CANDLES = Array.from({ length: 8 }, (_, i) => ({
  id: i,
  height: 30,
  isGreen: true,
  delay: i * 0.2,
...
const AUTH_STYLES = `
  ...
  .auth-price-update { animation: auth-price-flash 0.5s ease-out; }
...
const useMarketData = () => {
  ...
  const fetchMarketData = useCallback(async () => {
    try {
      ...
      const cryptoResponse = await fetch(
        `${CRYPTO_API_URL}/simple/price?ids=bitcoin,ethereum&vs_currencies=usd&include_24hr_change=true`
      );
      ...
      // Fetch stock data (using a free API - Alpha Vantage demo or similar)
      // For demo purposes, we'll simulate AAPL with a realistic price
      const aaplPrice = 178 + (Math.random() - 0.5) * 2;
      const aaplChange = (Math.random() - 0.5) * 2;

      const newTickers: TickerData[] = [
        {
          symbol: "BTC/USD",
          price: cryptoData.bitcoin?.usd?.toLocaleString('en-US', { maximumFractionDigits: 0 }) || "---",
          change: `${cryptoData.bitcoin?.usd_24h_change >= 0 ? '+' : ''}${cryptoData.bitcoin?.usd_24h_change?.toFixed(1) || 0}%`,
          isUp: (cryptoData.bitcoin?.usd_24h_change || 0) >= 0,
        },
        {
          symbol: "ETH/USD",
          price: cryptoData.ethereum?.usd?.toLocaleString('en-US', { maximumFractionDigits: 0 }) || "---",
          change: `${cryptoData.ethereum?.usd_24h_change >= 0 ? '+' : ''}${cryptoData.ethereum?.usd_24h_change?.toFixed(1) || 0}%`,
          isUp: (cryptoData.ethereum?.usd_24h_change || 0) >= 0,
        },
        {
          symbol: "EUR/USD",
          price: (1 / forexData.rates?.USD)?.toFixed(4) || "---",
        ...
        },
        {
          ...
          change: `${aaplChange >= 0 ? '+' : ''}${aaplChange.toFixed(1)}%`,
          isUp: aaplChange >= 0,
        },
      ];

      // Generate dynamic candles based on price movements
      const newCandles: CandleData[] = Array.from({ length: 8 }, (_, i) => {
        const btcChange = cryptoData.bitcoin?.usd_24h_change || 0;
        const baseHeight = 20 + Math.abs(btcChange) * 2;
        const randomFactor = 0.5 + Math.random();
        
        return {
          id: i,
          height: Math.min(70, Math.max(15, baseHeight * randomFactor + (i * 3) % 20)),
          isGreen: i % 2 === 0 ? btcChange >= 0 : Math.random() > 0.4,
          delay: i * 0.2,
      ...
      previousPrices.current = {
        'BTC/USD': cryptoData.bitcoin?.usd || 0,
        'ETH/USD': cryptoData.ethereum?.usd || 0,
      };
  ...
  useEffect(() => {
    ...
    // Set up polling interval (every 30 seconds to respect API limits)
    const interval = setInterval(fetchMarketData, 30000);

    return () => clearInterval(interval);
  }, [fetchMarketData]);

  return { tickers, candles, isLoading };
};
...
// Updated AnimatedTicker with real-time data
const AnimatedTicker = memo(({ tickers, isLoading }: { tickers: TickerData[]; isLoading: boolean }) => {
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      setUpdatedSymbols(symbols);
      const timer = setTimeout(() => setUpdatedSymbols(new Set()), 500);
      ...
      <div className="auth-ticker flex whitespace-nowrap py-1.5 sm:py-2">
        {[...tickers, ...tickers, ...tickers].map((ticker, i) => (
          <div 
            key={i} 
            className={cn(
              "flex items-center gap-2 sm:gap-4 md:gap-6 mx-4 sm:mx-6 md:mx-8 px-2 rounded transition-colors duration-300",
              updatedSymbols.has(ticker.symbol) && "auth-price-update"
            )}
          >
            <span className="text-gray-400 text-xs sm:text-sm font-medium">{ticker.symbol}</span>
            <span className={cn(
              "text-white text-xs sm:text-sm font-bold transition-all duration-300",
              isLoading && "animate-pulse"
            )}>
              {isLoading ? "---" : ticker.price}
            </span>
            <span className={cn(
              "text-xs font-semibold flex items-center gap-0.5 transition-all duration-300",
...
// Updated AnimatedCandlesticks with dynamic data
const AnimatedCandlesticks = memo(({ candles }: { candles: CandleData[] }) => (
  <div className="absolute bottom-0 left-0 right-0 h-24 sm:h-32 md:h-40 lg:h-48 hidden sm:flex items-end justify-center gap-2 md:gap-3 opacity-15 sm:opacity-20 overflow-hidden" style={{ contain: 'strict' }}>
    {candles.map((candle) => (
      <div
        key={candle.id}
        className="relative flex flex-col items-center auth-candle transition-all duration-500"
...
export default function Auth() {
  ...
  const { tickers, candles, isLoading: marketLoading } = useMarketData();
  ...
  return (
    <div className="auth-container auth-safe-area flex flex-col lg:flex-row relative overflow-hidden bg-[#0a0b0f] text-white selection:bg-emerald-500/30">
      ...
      <FloatingOrbs />
      <AnimatedGrid />
      <AnimatedTicker tickers={tickers} isLoading={marketLoading} />
      <AnimatedCandlesticks candles={candles} />
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 md:p-8 pt-14 sm:pt-16 lg:pt-8 relative z-10 auth-scroll overflow-y-auto">
        <Card 
          className={cn(
