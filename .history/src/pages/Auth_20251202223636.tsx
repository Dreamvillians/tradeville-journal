...
const FEATURES = [
  ...
  { icon: TrendingUp, title: "Strategy Tracking", description: "Identify profitable and losing setups." },
...
const AUTH_STYLES = `
  ...
  /* Custom scrollbar for the form on mobile */
...
const FloatingOrbs = memo(() => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ contain: 'strict' }}>
    {/* Simplified orbs for mobile, full for desktop */}
    ...
    {/* Particles - hidden on mobile, visible on tablet+ */}
...
const AnimatedTicker = memo(() => (
  <div className="absolute top-0 left-0 right-0 overflow-hidden bg-black/20 sm:bg-black/30 backdrop-blur-sm border-b border-white/5 z-20" style={{ contain: 'layout' }}>
    <div className="auth-ticker flex whitespace-nowrap py-1.5 sm:py-2">
      {[...TICKERS, ...TICKERS, ...TICKERS].map((ticker, i) => (
        <div key={i} className="flex items-center gap-2 sm:gap-4 md:gap-6 mx-4 sm:mx-6 md:mx-8">
          <span className="text-gray-400 text-[10px] sm:text-xs md:text-sm font-medium">{ticker.symbol}</span>
          <span className="text-white text-[10px] sm:text-xs md:text-sm font-bold">{ticker.price}</span>
          <span className={cn(
            "text-[9px] sm:text-[10px] md:text-xs font-semibold flex items-center gap-0.5",
            ticker.isUp ? "text-emerald-400" : "text-red-400"
          )}>
            {ticker.isUp ? <TrendingUp className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> : <TrendingDown className="w-2.5 h-2.5 sm:w-3 sm:h-3" />}
            {ticker.change}
          </span>
        </div>
      ))}
...
const AnimatedCandlesticks = memo(() => (
  <div className="absolute bottom-0 left-0 right-0 h-24 sm:h-32 md:h-40 lg:h-48 hidden sm:flex items-end justify-center gap-2 md:gap-3 opacity-15 sm:opacity-20 overflow-hidden" style={{ contain: 'strict' }}>
    {CANDLES.map((candle) => (
      <div
        key={candle.id}
        className="relative flex flex-col items-center auth-candle"
...
const AnimatedStat = memo(({ value, label }: { value: string; label: string }) => {
  ...
  return (
    <div className="p-2.5 sm:p-3 md:p-4 rounded-lg sm:rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 text-center group hover:bg-white/10 hover:border-emerald-500/30 transition-all duration-300">
      ...
      <div className="text-[10px] sm:text-xs md:text-sm text-gray-400">{label}</div>
    </div>
  );
});
...
const SocialButton = memo(({ 
  ...
  return (
    <div className="relative w-full group">
      {isLastUsed && (
        <div className="absolute -top-2.5 sm:-top-3 left-1/2 -translate-x-1/2 z-20 auth-bounce">
          <span className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-[8px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full shadow-lg flex items-center gap-0.5 sm:gap-1 whitespace-nowrap">
            <Activity className="w-2 h-2" />
            <span className="hidden xs:inline">Last used</span>
      ...
      <Button
        ...
        disabled={disabled}
        className={cn(
          "w-full h-9 sm:h-10 relative overflow-hidden transition-all duration-300",
    ...
    <div className="relative group">
      ...
      <div className="relative">
        ...
        <Input
          ...
          onBlur={handleBlur}
          className={cn(
            "h-10 sm:h-11 pl-10 text-sm sm:text-base",
...
const PasswordStrengthIndicator = memo(({ password, strength }: { 
  ...
  return (
    <div className="space-y-2 p-2.5 sm:p-3 bg-white/5 rounded-lg border border-white/5 auth-fade">
      <div className="flex items-center gap-2">
        ...
        <span className={cn(
          "text-[10px] font-bold uppercase min-w-[45px] text-right",
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2">
        {checks.map((item, i) => (
          <div key={i} className={cn(
            "flex items-center gap-1 text-[10px] sm:text-xs transition-colors duration-300 px-1.5 py-0.5 rounded",
...
export default function Auth() {
  ...
  return (
    <div className="auth-container auth-safe-area flex flex-col lg:flex-row relative overflow-hidden bg-[#0a0b0f] text-white selection:bg-emerald-500/30">
        ...
        >
          ...
          <div className="space-y-4 xl:space-y-6 auth-fade" style={{ animationDelay: '100ms' }}>
            <div className="flex items-center gap-3 xl:gap-4">
              ...
              <div>
                ...
                <div className="text-[10px] xl:text-xs text-emerald-400 font-medium tracking-wider uppercase">Trading Journal</div>
              </div>
            </div>
          ...
          >
            ...
            <div className="flex gap-1 mb-2 xl:mb-3">
              {[0, 1, 2, 3, 4].map((i) => (
                <svg key={i} className="w-3 h-3 xl:w-4 xl:h-4 text-yellow-400 fill-current" viewBox="0 0 20 20">
            ...
            <div className="mt-3 xl:mt-4 flex items-center gap-2 xl:gap-3">
              <div className="w-8 h-8 xl:w-10 xl:h-10 rounded-full bg-gradient-to-br from-emerald-500 to-blue-500 flex items-center justify-center text-white font-bold text-xs xl:text-sm">
                AM
              </div>
              <div>
                <p className="font-semibold text-white text-xs xl:text-sm">Alex M.</p>
                <p className="text-[10px] xl:text-xs text-gray-500">Forex Trader • 5 years</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-3 sm:p-4 md:p-6 lg:p-8 pt-12 sm:pt-14 md:pt-16 lg:pt-8 relative z-10 auth-scroll overflow-y-auto">
        <Card 
          className={cn(
            "w-full max-w-[400px] sm:max-w-md border-white/10 bg-[#12131a]/95 sm:bg-[#12131a]/90 backdrop-blur-xl shadow-2xl transition-all duration-700 transform relative overflow-hidden",
        ...
        >
          ...
          <CardHeader className="text-center pb-2 pt-6 sm:pt-8 px-4 sm:px-6 relative z-10">
            {/* Mobile/Tablet Logo */}
            <div className="lg:hidden flex flex-col items-center mb-4 sm:mb-6">
              <AnimatedLogo size="large" />
              <span className="mt-2 text-lg sm:text-xl font-bold text-white">Tradeville</span>
              <span className="text-[10px] text-emerald-400 uppercase tracking-wider">Trading Journal</span>
            </div>
          ...
          <CardContent className="pt-4 sm:pt-6 px-4 sm:px-6 pb-4 relative z-10">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid w-full grid-cols-2 bg-black/40 p-1 border border-white/5 rounded-lg h-9 sm:h-10">
                <TabsTrigger 
                  value="signin"
                  className="text-xs sm:text-sm data-[state=active]:bg-gradient-to-r data-[state=active]:from-emerald-500/20 data-[state=active]:to-blue-500/20 data-[state=active]:text-white text-gray-400 rounded-md transition-all duration-300"
                >
                  Sign In
                </TabsTrigger>
                <TabsTrigger 
              ...
              <TabsContent value="signin" className="space-y-4 mt-4 sm:mt-6 auth-fade">
                <form onSubmit={handleSignIn} className="space-y-3 sm:space-y-4">
                  <div className="space-y-1.5 sm:space-y-2">
                    <Label htmlFor="signin-email" className="text-gray-300 text-[10px] sm:text-xs uppercase tracking-wider font-semibold">
                      Email
                    </Label>
                    <AnimatedInput
                      id="signin-email"
                      ...
                      badge={lastUsedProvider === 'email' ? (
                        <div className="absolute right-3 top-1/2 -translate-y-1/2">
                          <span className="text-[8px] sm:text-[10px] bg-emerald-500/10 text-emerald-500 px-1.5 sm:px-2 py-0.5 rounded-full border border-emerald-500/20 animate-pulse">
                            Last
                          </span>
                        </div>
                      ) : undefined}
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="signin-password" className="text-gray-300 text-[10px] sm:text-xs uppercase tracking-wider font-semibold">
                        Password
                      </Label>
                      <button
                        type="button"
                        onClick={handleForgotPassword}
                        className="text-[10px] sm:text-xs text-emerald-400 hover:text-emerald-300 transition-colors touch-manipulation"
                      >
                        Forgot?
                      </button>
                    </div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="remember" className="text-xs sm:text-sm text-gray-400 cursor-pointer select-none">
                      Remember email
                    </label>
                  </div>
                  ...
                  <Button 
                    type="submit" 
                    className="w-full h-10 sm:h-11 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-semibold text-sm sm:text-base transition-all duration-300 group shadow-lg shadow-emerald-500/25 active:scale-[0.98] touch-manipulation relative overflow-hidden" 
                    disabled={loading}
                  >
                ...
                <div className="relative py-2 sm:py-3">
                  ...
                  <div className="relative flex justify-center text-[10px] sm:text-xs uppercase">
                    <span className="bg-[#12131a] px-2 sm:px-3 text-gray-500">Or continue with</span>
                  </div>
                </div>
                
                {/* Social Buttons - Responsive Grid */}
                <div className="space-y-2 sm:space-y-3">
                  <div className="grid grid-cols-5 gap-1.5 sm:gap-2 md:grid-cols-3 md:gap-3">
              ...
              <TabsContent value="signup" className="space-y-4 mt-4 sm:mt-6 auth-fade">
                <form onSubmit={handleSignUp} className="space-y-3 sm:space-y-4">
                  <div className="space-y-1.5 sm:space-y-2">
                    <Label htmlFor="signup-name" className="text-gray-300 text-[10px] sm:text-xs uppercase tracking-wider font-semibold">
                      Full Name
                    </Label>
                    <AnimatedInput
                      id="signup-name"
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                    <Label htmlFor="signup-email" className="text-gray-300 text-[10px] sm:text-xs uppercase tracking-wider font-semibold">
                      Email
                    </Label>
                    <AnimatedInput
                      id="signup-email"
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                    <Label htmlFor="signup-password" className="text-gray-300 text-[10px] sm:text-xs uppercase tracking-wider font-semibold">
                      Password
                    </Label>
                    <AnimatedInput
                      id="signup-password"
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    />
                    <label htmlFor="terms" className="text-xs sm:text-sm text-gray-400 cursor-pointer leading-relaxed">
                      I agree to the{" "}
                ...
                <div className="relative py-2 sm:py-3">
                  ...
                  <div className="relative flex justify-center text-[10px] sm:text-xs uppercase">
                    <span className="bg-[#12131a] px-2 sm:px-3 text-gray-500">Or sign up with</span>
                  </div>
                </div>
                ...
                <div className="flex justify-center pt-1">
                  <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-emerald-500/10 to-blue-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] sm:text-xs font-medium">
                    <Zap className="w-3 h-3" />
                    <span className="hidden xs:inline">14-day free trial • No card required</span>
                    <span className="xs:hidden">14-day free trial</span>
                  </div>
                </div>
              </TabsContent>
            </Tabs>
          ...
          <div className="bg-black/30 py-2 sm:py-3 border-t border-white/5 flex items-center justify-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-gray-500 relative z-10 rounded-b-xl">
