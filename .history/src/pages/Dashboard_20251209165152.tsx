...
// Profit Factor Mini Chart Component with Trader Level
const ProfitFactorChart = memo(({ profitFactor }: { profitFactor: number }) => {
  ...
  // Determine Trader Level based on Profit Factor
  const getLevel = (pf: number) => {
    if (pf >= 3.0) return { label: "Legendary", icon: Sparkles, color: "text-purple-400" };
    if (pf >= 2.0) return { label: "Elite", icon: Award, color: "text-emerald-400" };
    if (pf >= 1.5) return { label: "Professional", icon: TrendingUp, color: "text-blue-400" };
    if (pf >= 1.0) return { label: "Profitable", icon: Shield, color: "text-yellow-400" };
    return { label: "Novice", icon: BookOpen, color: "text-muted-foreground" };
  };

  const level = getLevel(profitFactor);
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="mt-1 pt-1.5 border-t border-border/40 flex items-center justify-between">
        ...
        <div className={cn("flex items-center gap-1", level.color)}>
           ...
           <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wider">{level.label}</span>
        </div>
      </div>
    </div>
  );
