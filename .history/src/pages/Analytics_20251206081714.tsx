...
const TraderLevel = memo(function TraderLevel({
  ...
  const { level, color, progress } = useMemo(() => {
    if (totalTrades < 10) {
      return { level: "Beginner", color: "gray" as const, progress: Math.min(totalTrades * 10, 100) };
    }
...
const EquityCurveChart = memo(function EquityCurveChart({
  ...
  const range = rawRange === 0 ? (Math.abs(maxValue) * 0.2 || 100) : rawRange;
...
const AnalyticsContent = memo(function AnalyticsContent({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Strategy performance + summary using full grid (1 column) */}
...
const PeriodAnalytics = memo(function PeriodAnalytics({
  ...
  // Memoize this result so we don't recalculate for "Yearly" when looking at "Weekly"
