...
const getUserDisplayName = (
  ...
  // Priority: profile display_name > profile full_name > profile first_name > 
  // metadata full_name > metadata first_name > metadata name > "Trader"
...
const ProfitFactorChart = memo(({ profitFactor }: { profitFactor: number }) => {
  ...
  // Determine Trader Level Logic
  const getLevelInfo = (pf: number) => {
    if (pf >= 3.0) return { label: "Legendary", color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/20", icon: Sparkles };
    if (pf >= 2.0) return { label: "Elite", color: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20", icon: Award };
    if (pf >= 1.5) return { label: "Professional", color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20", icon: TrendingUp };
    if (pf >= 1.0) return { label: "Profitable", color: "text-green-400", bg: "bg-green-500/10", border: "border-green-500/20", icon: Shield };
    return { label: "Novice", color: "text-muted-foreground", bg: "bg-muted/10", border: "border-border/40", icon: BookOpen };
  };

  const level = getLevelInfo(profitFactor);
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {/* Styled Trader Level Indicator */}
      <div className="mt-2 pt-2 border-t border-border/40">
        <div className={cn("flex items-center gap-2 rounded-lg p-1.5 sm:p-2 border transition-all duration-500", level.bg, level.border)}>
           <div className={cn("p-1 rounded-md bg-background/60 backdrop-blur-sm shadow-sm", level.color)}>
             <LevelIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
           </div>
           <div className="flex flex-col min-w-0">
             <span className="text-[8px] sm:text-[9px] text-muted-foreground uppercase tracking-wider font-semibold leading-none mb-0.5">
               Current Rank
             </span>
             <span className={cn("text-[10px] sm:text-xs font-bold leading-none truncate", level.color)}>
               {level.label}
             </span>
           </div>
        </div>
