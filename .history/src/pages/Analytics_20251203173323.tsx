...
// -------------------------------------------------------------------------------------
// Performance summary block (updated All‑Time card to ONE ROW)
// -------------------------------------------------------------------------------------
...
const PerformanceSummary = memo(function PerformanceSummary({
  ...
  // -----------------------------------------------------------
  // Special "All-Time" Layout: One Row / Banner Style
  // -----------------------------------------------------------
  if (isAllTime) {
    return (
      <Card className="analytics-glass analytics-card-hover relative overflow-hidden w-full">
        ...
        <CardContent className="relative z-10 p-5">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 lg:gap-10">
            ...
            {/* 1. Primary Metric: Net P&L */}
            <div className="flex flex-col min-w-[140px]">
               <span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mb-1">
                  Lifetime Net P&L
               </span>
               <div className="flex items-baseline gap-2">
                  <span className={cn("text-3xl font-bold tracking-tight", metrics.netPnL >= 0 ? "text-emerald-400" : "text-red-400")}>
            ...
            <div className="flex flex-wrap items-center gap-x-8 gap-y-4 flex-1">
                ...
                <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Win Rate</span>
                    <div className="flex items-end gap-1.5">
                        <span className="text-xl font-semibold text-white">{metrics.winRate.toFixed(1)}%</span>
                        <span className="text-[10px] text-gray-500 mb-1">({metrics.totalTrades} trades)</span>
                    </div>
                </div>

                {/* Profit Factor */}
                <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Profit Factor</span>
                    <span className={cn("text-xl font-semibold", metrics.profitFactor >= 1.5 ? "text-emerald-400" : "text-white")}>
                ...
                {/* Expected Value */}
                <div className="flex flex-col">
                    <span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mb-0.5">Expectancy</span>
                    <span className={cn("text-xl font-semibold", metrics.expectedValue > 0 ? "text-emerald-400" : "text-gray-300")}>
                        {formatCurrency(metrics.expectedValue)}
                    </span>
                </div>
            </div>

            {/* 3. Visual: Trade Distribution Bar (Mini) */}
            <div className="flex flex-col min-w-[180px] w-full lg:w-auto gap-2">
                ...
                <div className="flex h-2.5 w-full rounded-full overflow-hidden bg-gray-800">
                    <div 
                        ...
                        style={{ width: `${metrics.profitableTrades > 0 ? (metrics.profitableTrades / metrics.totalTrades) * 100 : 0}%` }}
                    />
                    <div 
                        className="h-full bg-red-500 transition-all duration-1000" 
                        style={{ width: `${metrics.nonProfitableTrades > 0 ? (metrics.nonProfitableTrades / metrics.totalTrades) * 100 : 0}%` }}
                    />
                </div>
...
const AnalyticsContent = memo(function AnalyticsContent({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Strategy performance + summary (Updated Layout) */}
      <div className="flex flex-col gap-4">
         {/* Full width for All-Time Summary if periodLabel is All Time */}
         <div className="analytics-slide-up" style={{ animationDelay: "770ms" }}>
            <PerformanceSummary
                metrics={metrics}
         ...
         {/* Strategy Perf below it */}
         <div className="analytics-slide-up" style={{ animationDelay: "820ms" }}>
