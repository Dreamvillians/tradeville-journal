...
const StatCard = memo(({ 
  ...
  return (
    <Card 
      className={cn(
        "dash-glass dash-card-hover relative overflow-hidden group",
        size === 'large' && "p-2"
      )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                ...
                <span className="text-xs text-gray-400">{stats.totalTrades} trades this week</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                <Flame className="w-3 h-3 text-orange-400" />
                <span className="text-xs text-gray-400">{stats.streak} day streak</span>
              </div>
            </div>
          </div>
...
const PerformanceChart = memo(({ trades }: { trades: Trade[] }) => {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = days; i >= 0; i--) {
      ...
      const dailyPnL = (Math.random() - 0.45) * 500; // Simulated P&L
      cumulative += dailyPnL;
      ...
      <CardHeader className="relative z-10 pb-2">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <CardDescription className="text-gray-500">Portfolio performance over time</CardDescription>
          </div>
        ...
        <div className="relative h-64 mt-4">
          ...
          <div className="ml-16 h-full relative">
            ...
            <div className="absolute inset-0 flex items-end">
              {chartData.filter((_, i) => i % Math.ceil(chartData.length / 20) === 0).map((d, i, filtered) => (
                <div
                  key={i}
                ...
                >
                  {hoveredBar === i && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 bg-[#1a1b23] border border-white/10 rounded-lg shadow-xl z-10 whitespace-nowrap">
                      ...
                      <p className="text-sm font-bold text-white">{formatCurrency(d.value)}</p>
                      <p className={cn(
                        "text-xs font-medium",
      ...
      <CardHeader className="relative z-10 pb-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-gray-500">Your latest trades</CardDescription>
          </div>
        ...
        <div className="space-y-3">
          {recentTrades.length > 0 ? recentTrades.map((trade, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{trade.strategy || 'Manual Trade'}</p>
                </div>
              </div>
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500">{getTimeAgo(trade.closed_at)}</p>
              </div>
            </div>
          )) : (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500">Start logging your trades to see them here</p>
            </div>
          )}
        </div>
      </CardContent>
        ...
        <div className="grid grid-cols-2 gap-3">
          {actions.map((action, index) => (
            <Button
              ...
              className={cn(
                "h-auto py-4 flex flex-col items-center gap-2 group transition-all",
                action.primary 
...
const TradingGoals = memo(({ stats }: { stats: any }) => {
  const goals = [
    { label: "Monthly P&L Target", current: 2450, target: 5000, color: "emerald" },
    { label: "Win Rate Goal", current: 62, target: 70, unit: "%", color: "blue" },
    { label: "Trades This Month", current: 24, target: 40, color: "purple" },
  ];
...
const PerformanceMetrics = memo(({ stats }: { stats: any }) => {
  const metrics = [
    { label: "Profit Factor", value: stats.profitFactor.toFixed(2), icon: TrendingUp, trend: "up" },
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {metrics.map((metric, index) => (
            ...
            >
              ...
              <p className={cn(
                "text-lg font-bold",
                metric.trend === "up" ? "text-emerald-400" : metric.trend === "down" ? "text-red-400" : "text-white"
              )}>
                {metric.value}
              </p>
            </div>
...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      bestTrade,
      worstTrade,
      streak: 5, // Would calculate from actual data
    };
  }, [trades]);
    ...
    <div className="min-h-screen bg-[#0a0b0f] text-white relative overflow-x-hidden">
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" className="bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          {/* Welcome Card - Full width on mobile, spans 2 cols on lg */}
          <div className="md:col-span-2 lg:col-span-2 dash-slide-up" style={{ animationDelay: '0ms' }}>
          ...
          <div className="md:col-span-2 lg:col-span-3 dash-slide-up" style={{ animationDelay: '200ms' }}>
          ...
          <div className="md:col-span-2 dash-slide-up" style={{ animationDelay: '500ms' }}>
          ...
          {/* Performance Metrics */}
          <div className="dash-slide-up" style={{ animationDelay: '600ms' }}>
