      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-gray-200">{formatPrice(trade.entry_price)}</span>
          </div>
          {trade.exit_price && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-gray-500 font-mono">OUT</span>
              <span className="font-mono text-gray-200">{formatPrice(trade.exit_price)}</span>
            </div>
          )}
          {trade.position_size && (
            <div className="text-[10px] text-gray-600">Size: {trade.position_size}</div>
          )}
        </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/5">
        <div className="flex gap-2">
          {beforeImage && (
            ...
            >
              <img src={beforeImage.url} className="w-full h-full object-cover opacity-60 hover:opacity-100 transition-opacity" alt="Before" />
            </div>
          )}
          {afterImage && (
            <div 
            ...
            >
              <img src={afterImage.url} className="w-full h-full object-cover opacity-60 hover:opacity-100 transition-opacity" alt="After" />
            </div>
          )}
            ...
            <Card className="journal-glass border-white/10">
              <CardHeader className="border-b border-white/5 pb-4 px-4 sm:px-6">
                <div className="flex justify-between items-center">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setShowForm(false)} className="text-gray-500 hover:text-white">
                    Cancel
                  </Button>
                </div>
              </CardHeader>
                            ...
                            <TableCell className="pr-6">
                              <div className="flex justify-center gap-2">
                                {beforeImage && (
                                  ...
                                  >
                                    <img src={beforeImage.url} className="w-full h-full object-cover opacity-60 group-hover/img:opacity-100 transition-opacity" alt="Before" />
                                  </div>
                                )}
                                {afterImage && (
                                  <div 
                                  ...
                                  >
                                    <img src={afterImage.url} className="w-full h-full object-cover opacity-60 group-hover/img:opacity-100 transition-opacity" alt="After" />
                                  </div>
                                )}
                                {!beforeImage && !afterImage && (
                                  <div className="w-8 h-8 rounded bg-white/2 border border-white/5 flex items-center justify-center" title="No screenshots">
                                    <ImageIcon className="w-3 h-3 text-gray-700" />
                                  </div>
                                )}
                              </div>
      ...
      <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
        <DialogContent className="w-[95vw] max-w-5xl bg-black/95 border-white/10 p-2 sm:p-0 overflow-hidden">
          <DialogHeader className="absolute top-2 left-2 sm:top-4 sm:left-4 z-10">
            <DialogTitle className="text-white drop-shadow-md bg-black/50 px-2 py-1 rounded text-sm sm:text-base">Trade Screenshot</DialogTitle>
          </DialogHeader>
          {selectedImage && (
