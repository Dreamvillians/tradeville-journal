      ...
      <CardContent className="relative z-10 pt-2 sm:pt-4 px-3 sm:px-6 pb-3 sm:pb-6">
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
          <div className="p-2 sm:p-3 rounded-lg bg-white/5">
            ...
            <p className="text-sm sm:text-lg font-bold text-emerald-400">+24.5%</p>
          </div>
          <div className="p-2 sm:p-3 rounded-lg bg-white/5">
            <p className="text-[10px] sm:text-xs text-gray-500 mb-0.5 sm:mb-1">Max DD</p>
            <p className="text-sm sm:text-lg font-bold text-red-400">-8.2%</p>
          </div>
          <div className="p-2 sm:p-3 rounded-lg bg-white/5">
            <p className="text-[10px] sm:text-xs text-gray-500 mb-0.5 sm:mb-1">Sharpe</p>
            <p className="text-sm sm:text-lg font-bold text-white">1.85</p>
          </div>
        </div>
...
// Trading Goals
const TradingGoals = memo(({ stats }: { stats: DashboardStats }) => {
  const goals = [
    {
      label: "Monthly P&L",
      current: 2450,
    ...
    { label: "Win Rate", current: parseFloat(stats.winRate), target: 70, unit: "%", color: "blue" },
    {
      label: "Trades/Month",
      current: 24,
      target: 40,
...
const Dashboard = () => {
  ...
  const stats = useMemo((): DashboardStats => {
    ...
    // Decisive trades = wins + losses (excludes break-even)
    ...
    // Only considers trades with a definitive outcome (win or loss)
    const winRate = decisiveTrades > 0 
      ? (wins / decisiveTrades) * 100 
      : 0;

    // Average win (only winning trades)
    ...
    // Average loss (only losing trades, excludes break-even)
    ...
    // Expectancy (per trade, includes all trades)
          ...
          <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute -top-1 -right-1 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-emerald-500 flex items-center justify-center">
                <span className="text-[8px] sm:text-[10px] font-bold text-white">3</span>
              </div>
            </div>
            <div className="min-w-0">
