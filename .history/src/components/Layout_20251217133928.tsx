...
// Ticker Styles - Animation definition
const TICKER_STYLES = `
  ...
  @media (prefers-reduced-motion: reduce) {
    .dash-ticker { animation: none !important; transform: none; }
  }
`;
...
const MarketTicker = memo(() => {
  ...
  // Duplicate data to ensure smooth infinite loop transition
  const displayData = useMemo(() => [...data, ...data, ...data, ...data, ...data, ...data], [data]);

  return (
    <div className="fixed top-0 left-0 right-0 z-50 h-10 w-full overflow-hidden bg-black/60 backdrop-blur-xl border-b border-white/5 select-none pointer-events-none">
      <div className="dash-ticker flex h-full w-max items-center whitespace-nowrap will-change-transform">
...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <SidebarProvider>
      <div className="relative flex min-h-screen w-full flex-col bg-[#05060b] text-foreground overflow-hidden">
        
        {/* Fixed Ticker Overlay */}
        <MarketTicker />
        
        {/* 
          Main Layout Wrapper 
          - pt-10 pushes content down to account for fixed ticker 
          - overflow-hidden ensures no scroll on this wrapper itself
          - h-screen bounds the layout to the viewport
        */}
        <div className="flex h-screen w-full pt-10 overflow-hidden">
          
          <AppSidebar />
          
          {/* 
            Content Area 
            - min-w-0 is Critical for Flex children to prevent overflow 
            - max-w-full ensures it doesn't exceed parent
          */}
