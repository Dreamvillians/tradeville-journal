...
import { LogOut, CalendarDays, Menu, X } from "lucide-react";
import {
  ReactNode,
  useEffect,
  useMemo,
...
function LayoutInner({ children }: LayoutProps) {
  ...
  // sidebar state (provided by SidebarProvider)
  ...
  // one source of truth for "is sidebar open"
  ...
  return (
    <div className="flex h-screen w-full bg-[#05060b] text-foreground overflow-hidden">
      ...
      {/* FLOATING MOBILE TOGGLE (OPEN/CLOSE) */}
      <Button
        type="button"
        onClick={toggleSidebar}
        className={cn(
          "fixed left-3 top-3 z-50 h-9 w-9 rounded-full",
          "md:hidden shadow-lg border border-border/70",
      ...
      <div className="flex flex-1 flex-col h-full min-w-0 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3 flex-1 min-w-0">
            {/* Desktop trigger stays in header */}
