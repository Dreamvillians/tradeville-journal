...
const MarketTicker = memo(() => {
  ...
  return (
    // UPDATED: Removed 'fixed' positioning. Now it's a standard block element in the flex column.
    // 'shrink-0' ensures it doesn't collapse if vertical space is tight.
    <div className="w-full h-10 shrink-0 overflow-hidden bg-black/60 backdrop-blur-xl border-b border-white/5 select-none pointer-events-none z-50">
...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <SidebarProvider>
      {/* 
        LAYOUT FIX:
        1. 'h-screen' bounds the entire app to the window height.
        2. 'flex-col' stacks the Ticker on top of the Workspace content.
        3. Removed 'pt-10' and fixed positioning, which causes the gaps.
      */}
      <div className="flex h-screen w-full flex-col bg-[#05060b] text-foreground overflow-hidden">
        
        {/* 1. Ticker (Static at top) */}
        <MarketTicker />
        
        {/* 2. Workspace Area (Sidebar + Main Content) - Fills remaining height */}
