      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.data) {
            ...
            const cfg = BINANCE_SYMBOLS[symbol.toLowerCase()];
            const id = cfg?.id;
            if (id && price) {
              this.notify("crypto", {
                id,
...
// =====================================================================================
// ASSET CONFIGURATIONS – “Most traded / need-to-know” list
// =====================================================================================

// Crypto majors (Binance) – what most traders watch first
const BINANCE_SYMBOLS: Record<string, BinanceSymbolConfig> = {
  btcusdt: {
    id: "bitcoin",
...
// Stocks & ETFs – indices first (market view), then mega‑cap leaders
const STOCK_SYMBOLS = [
  // Broad market ETFs users care about
  { id: "spy", symbol: "SPY", name: "S&P 500 ETF", exchange: "NYSE" },
  { id: "qqq", symbol: "QQQ", name: "Nasdaq 100 ETF", exchange: "NASDAQ" },
  { id: "dia", symbol: "DIA", name: "Dow Jones ETF", exchange: "NYSE" },

  // Big tech & high‑volume names
  ...
  { id: "meta", symbol: "META", name: "Meta Platforms", exchange: "NASDAQ" },
];

// Major FX pairs + the key crosses most traders watch
const FOREX_PAIRS = [
  // USD majors – global benchmarks
  { id: "eurusd", symbol: "EUR/USD", name: "Euro / US Dollar" },
  { id: "gbpusd", symbol: "GBP/USD", name: "British Pound / US Dollar" },
  { id: "usdjpy", symbol: "USD/JPY", name: "US Dollar / Japanese Yen" },
  { id: "audusd", symbol: "AUD/USD", name: "Australian Dollar / US Dollar" },
  { id: "nzdusd", symbol: "NZD/USD", name: "New Zealand Dollar / US Dollar" },
  { id: "usdcad", symbol: "USD/CAD", name: "US Dollar / Canadian Dollar" },
  { id: "usdchf", symbol: "USD/CHF", name: "US Dollar / Swiss Franc" },

  // Popular crosses – risk & carry sentiment
  { id: "eurjpy", symbol: "EUR/JPY", name: "Euro / Japanese Yen" },
  { id: "gbpjpy", symbol: "GBP/JPY", name: "British Pound / Japanese Yen" },
  { id: "eurgbp", symbol: "EUR/GBP", name: "Euro / British Pound" },
];

// Core commodities – what macro & traders watch daily
const COMMODITIES = [
...
const createInitialData = (): MarketData[] => {
  // Crypto majors
  const cryptos: MarketData[] = Object.entries(BINANCE_SYMBOLS).map(
    ([, cfg]) => ({
      id: cfg.id,
...
export const useMarketData = () => {
  ...
  const updatePrice = useCallback(
    (id: string, newPrice: number, changePercent?: number) => {
      ...
      setData((prev) =>
        prev.map((item) => {
          if (
            ...
            const previousPrice =
              lastPricesRef.current[item.id] || item.price || newPrice;
            ...
            if (
              previousPrice > 0 &&
              Math.abs(newPrice - previousPrice) / previousPrice > 0.0001
            ) {
              setPriceFlash((f) => ({
                ...f,
                [item.id]: isUp ? "up" : "down",
              }));
              setTimeout(
                () =>
  ...
  useEffect(() => {
    ...
    const fetchForexData = async () => {
      ...
      try {
        const symbols = FOREX_PAIRS.map((f) =>
          f.symbol.replace("/", "")
        ).join(",");
    ...
    // Backup crypto prices via CoinGecko
    const fetchCoinGeckoData = async () => {
      try {
        const ids = Object.values(BINANCE_SYMBOLS)
          .map((cfg) => cfg.id)
          .join(",");
        const response = await fetch(
    ...
    const forexInterval = setInterval(
      fetchForexData,
      CONFIG.TWELVE_DATA_INTERVAL
    );
    const coingeckoInterval = setInterval(
      fetchCoinGeckoData,
      CONFIG.COINGECKO_INTERVAL
    );
    ...
    return () => {
      ...
      // WebSockets remain managed by singleton
    };
  }, [updatePrice]);
...
const TickerItem = memo(
  ({ item, flash }: { item: MarketData; flash: "up" | "down" | null }) => {
  ...
  return (
    ...
    >
        ...
        <div className="hidden lg:flex items-center gap-1 ml-1 pl-2 border-l border-white/10">
          <div
            className={cn(
              ...
              connectionStatus.binance ? "bg-emerald-500" : "bg-gray-600"
            )}
          ...
          <div
            className={cn(
              "w-1.5 h-1.5 rounded-full",
              connectionStatus.finnhub ? "bg-emerald-500" : "bg-gray-600"
            )}
            title="Finnhub (Stocks/ETFs)"
          />
          <div
            className={cn(
              "w-1.5 h-1.5 rounded-full",
              connectionStatus.twelveData ? "bg-emerald-500" : "bg-gray-600"
            )}
          ...
          <div
            className={cn(
              "w-1.5 h-1.5 rounded-full",
              connectionStatus.alphaVantage ? "bg-emerald-500" : "bg-gray-600"
            )}
          ...
          <div
            className={cn(
              "w-1.5 h-1.5 rounded-full",
              connectionStatus.coingecko ? "bg-emerald-500" : "bg-gray-600"
            )}
            title="CoinGecko (Crypto backup)"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        <div
          className={cn(
            "flex whitespace-nowrap items-center h-full",
            !isPaused && "dash-ticker"
          )}
        >
